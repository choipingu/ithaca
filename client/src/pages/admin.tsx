...
function Admin(): JSX.Element {
    ...
    const getReported = async () => {
        const reportedresult = await axios.get(`${URL}/report/board`, config)
    ...
    const getReportedComment = async () => {
        const reportedcommentresult = await axios.get(`${URL}/report/comment`, config)
    ...
    const handlePanalty = (user: string, contentid: string) => {
        if (penalty === '혐의없음') {
            ...
            axios.patch(`${URL}/board/report`, { contentId: contentid, initialrize: true }, config)
            ...
            axios.delete(`${URL}/board/${contentid}`, config)
                .then(el => setNum(restnum - 1))
            ...
            // console.log(user)
            axios.patch(`${URL}/report`, { userId: user, penalty: 7 }, config)
                .then(el => {
                    axios.delete(`${URL}/board/${contentid}`, config)
                })
                .then(el => setNum(restnum - 1))
            ...
            axios.patch(`${URL}/report`, { userId: user, penalty: 30 }, config)
                .then(el => {
                    axios.delete(`${URL}/board/${contentid}`, config)
                })
                .then(el => setNum(restnum - 1))
            ...
            axios.patch(`${URL}/report`, { userId: user, penalty: 365 }, config)
                .then(el => {
                    axios.delete(`${URL}/board/${contentid}`, config)
                })
                .then(el => setNum(restnum - 1))
    ...
    const handleCommentPanalty = (user: string, commentid: string) => {
        if (penalty === '혐의없음') {
            //해당 글의 신고된 횟수만 0으로 만들고 종료, 
            axios.patch(`${URL}/comment/report`, { commentId: commentid, initialrize: true }, config)
                .then(el => setNum(restnum - 1))
                .then(el => alert('처리되었습니다.'))
            ...
            //해당 글만 삭제함 axios delete board/:contentId 사용하면 됨 
            axios.delete(`${URL}/comment/${commentid}`, config)
                .then(el => setNum(restnum - 1))
                .then(el => alert("삭제되었습니다"))
        }
