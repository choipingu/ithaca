...
function MyPageEdit () {
  ...
  const handleOnBlur = (event: { target: { placeholder: string; value: string; }; }) => {
    if (event.target.placeholder === 'nickname') {
      if (regNickname.test(event.target.value)) {
        axios
          .post('http://3.15.16.232:8080/check', { nickname: inputInfo.nickname }, config)
          .then((res) => {
    ...
    if (event.target.placeholder === 'password') {
      axios
        .post('http://3.15.16.232:8080/check', { password: inputInfo.password }, config)
        .then(res => {
  ...
  const handleModify = () => {
    ...
    checkText.password === '확인되었습니다.') {
      axios.patch('http://3.15.16.232:8080/user', inputInfo, config).then((res) => {
        // 마이페이지로 리다이렉트
        navigate('/mypage');
      }).catch(err => {
  ...
  return (
    <MyPageEditWrap>
      <MyPageEditContainer>
        ...
        <InputWrap>
          <MyPageEditInput type='email' placeholder='not' disabled />
